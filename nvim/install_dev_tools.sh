...
case "${platform_string}" in
  ...
  MSYS_NT*) OS_TYPE="msys2" ;;
  *)        OS_TYPE="unknown" ;;
esac

# TODO - Look up correct package names to add Windows MSYS2 support
if [ "${OS_TYPE}" = "msys2" ] || [ "${OS_TYPE}" = "unknown" ]; then
  echo -e "Unsupported platform: ${platform_string}\n" >&2
  return 1
fi
